...
class TestEntityLookup:
    ...
    def test_lookup_after_connect(self, client: EspHomeClient) -> None:
        from aioesphomeapi import SensorInfo

        info = SensorInfo(object_id="temp_offset", key=4242, name="x")
        client._entities_by_object_id = {"temp_offset": info}
        assert client.entity_info("temp_offset") is info
        assert client.entity_object_ids == ("temp_offset",)

    def test_unknown_object_id_is_none(self, client: EspHomeClient) -> None:
        assert client.entity_info("nope") is None

    def test_empty_while_disconnected(self, client: EspHomeClient) -> None:
        assert client.entity_object_ids == ()
        assert client.entity_info("anything") is None
