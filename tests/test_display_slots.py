...
class TestIdempotentWrites:
    ...
    async def test_forget_lets_an_unchanged_value_through(self) -> None:
        """A rebooted node holds nothing, so dedupe must stop vouching for it."""
        rec = Recorder()
        writer = _writer(rec)
        writer.write("homey_t_line1", "Bedroom", kind="text")
        await asyncio.sleep(0.05)
        # normally suppressed: the node is believed to still hold this
        writer.write("homey_t_line1", "Bedroom", kind="text")
        await asyncio.sleep(0.05)
        assert [n for n, _ in rec.calls].count("homey_set_text") == 1

        writer.forget_node_state()
        writer.write("homey_t_line1", "Bedroom", kind="text")
        await asyncio.sleep(0.05)
        assert [n for n, _ in rec.calls].count("homey_set_text") == 2

    async def test_forget_sends_nothing_by_itself(self) -> None:
        """Refilling is the schedule's job, so a reconnect costs no refresh."""
        rec = Recorder()
        writer = _writer(rec)
        writer.write("homey_t_line1", "Bedroom", kind="text")
        await asyncio.sleep(0.05)
    ...
    async def test_forget_leaves_a_queued_write_alone(self) -> None:
        """A Flow that fired while the node was away must still go out."""
        rec = Recorder(fail_times=1)
        writer = _writer(rec)
