...
class DisplaySlotWriter:
    ...
    def forget_node_state(self) -> None:
        """Drop the record of what the node holds, after it reconnects.

        A node that rebooted comes back with empty slots, so the record is no
        longer true. It matters because `write` dedupes against it: left in
        place, a value that has not changed since the reboot is judged already
        drawn and never sent, and the tile stays blank for good rather than
        until the next write.

        Clearing it lets the next write through whatever its value, so the
        panel refills on the writer's normal schedule instead of costing a
        reconnect its own three-second refresh.
        """
        self._written.clear()
